...
//Testing calculate edit distance function
TEST(TrojanMapTest, CalculateEditDistance) 
{
  ...
  EXPECT_EQ(m.CalculateEditDistance("geek", "gesek"), 1);
  EXPECT_EQ(m.CalculateEditDistance("cat", "cut"), 1);
  EXPECT_EQ(m.CalculateEditDistance("Sunday", "Saturday"), 3);
  EXPECT_EQ(m.CalculateEditDistance("food", "money"), 4);
  EXPECT_EQ(m.CalculateEditDistance("voldemort", "dumbledore"), 7);
  EXPECT_EQ(m.CalculateEditDistance("cpp",""),3);
...
TEST(TrojanMapTest, FindClosestName) 
{
  ...
  EXPECT_EQ(m.FindClosestName("FedEx Prnt and Shop Center"), "FedEx Print and Ship Center");
}
...
/////////////////////////////////////////////////////////////
TEST(TrojanMapTest, CalculateShortestPath_Dijkstra_2) {
  TrojanMap m;
  
  // Test from Target to Ralphs
  auto path = m.CalculateShortestPath_Dijkstra("Target","Ralphs");
  std::vector<std::string> gt{
      
"5237417650","6814769289","6813379584","6813379479","4015372485","7071032399","4015372486","6813405232","122719216","6813405229","4015372487","4015372488","6813405266","6813416159","122814447","4015377689","4015377690","6816193698","6807937306","6807937309","6804883323",
"6816193696","544693739","4015377691","6816193694","6816193693","6816193692","4015442011","6787470576","6816193770","123230412",
"452688931","452688933","6816193774","123408705","6816193777","452688940","123318563","6813416129","6813416130","7645318201","6813416131",
"8410938469","6805802087","4380040167","4380040158","4380040154","2578244375"}; 
  // Print the path lengths
  std::cout << "My path length: "  << m.CalculatePathLength(path) << "miles" << std::endl;
  std::cout << "GT path length: " << m.CalculatePathLength(gt) << "miles" << std::endl;
  EXPECT_EQ(path, gt);
  ...
  path = m.CalculateShortestPath_Dijkstra("Ralphs", "Target");
  std::reverse(gt.begin(),gt.end()); // Reverse the path
...
TEST(TrojanMapTest, CalculateShortestPath_Dijkstra_3) {
  TrojanMap m;
  
  // Test from Ralphs to Chick-fil-A
  auto path = m.CalculateShortestPath_Dijkstra("Trader Joes","Target");
  std::vector<std::string> gt{
      "5237417649", "9591449485", "6396649383", "6814769289", "5237417650" }; 
  // Print the path lengths
  std::cout << "My path length: "  << m.CalculatePathLength(path) << "miles" << std::endl;
  std::cout << "GT path length: " << m.CalculatePathLength(gt) << "miles" << std::endl;
  EXPECT_EQ(path, gt);
  ...
  // Reverse the input from Ralphs to Chick-fil-A
  path = m.CalculateShortestPath_Dijkstra("Target", "Trader Joes");
  std::reverse(gt.begin(),gt.end()); // Reverse the path
...
TEST(TrojanMapTest, CalculateShortestPath_Bellman_Ford_2) {
  TrojanMap m;
  
  // Test from Ralphs to Chick-fil-A
  auto path = m.CalculateShortestPath_Bellman_Ford("Target", "Trader Joes");
  std::vector<std::string> gt{
      "5237417650", "6814769289", "6396649383", "9591449485", "5237417649"}; // Expected path
  // Print the path lengths
  std::cout << "My path length: "  << m.CalculatePathLength(path) << "miles" << std::endl;
  std::cout << "GT path length: " << m.CalculatePathLength(gt) << "miles" << std::endl;
  EXPECT_EQ(path, gt);
  ...
  // Reverse the input from Ralphs to Chick-fil-A
  path = m.CalculateShortestPath_Bellman_Ford("Trader Joes", "Target");
  std::reverse(gt.begin(),gt.end()); // Reverse the path
...
TEST(TrojanMapTest, CycleDetection) {
  ...
  EXPECT_EQ(result2, false);
   ...
   // Test case 3
  std::vector<double> square3 = {-118.290, -118.288, 34.030, 34.025};
...
TEST(TrojanMapTest, FindNearby_2) {
  ...
  auto result = m.FindNearby("supermarket", "Target", 10, 10);
...
TEST(TrojanMapTest, FindNearby_3) {
  ...
  auto result = m.FindNearby("supermarket", "Food 4 Less", 10, 10);
  std::vector<std::string> ans{"2578244375", "5237417649", "6045067406"};
  EXPECT_EQ(result, ans);
}