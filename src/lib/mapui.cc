...
void MapUI::PrintMenu() {
  ...
  case '3':
  {
    ...
    if (results.size() != 0) {
      ...
      std::cout << "\nThe route distance is calculated as:" << map.CalculatePathLength(results) << " miles" << std::endl;
      PlotPath(results);
    } else {
  ...
  case '4':
  {
    ...
    if (results.second.size() != 0) {
      ...
      std::cout << "\nThe route distance is calculated as:" << results.first << " miles" << std::endl;
    ...
    if (results.second.size() != 0) {
      for (auto x : results.second[results.second.size()-1]) std::cout << "\"" << x << "\",";
      std::cout << "\nThe route distance is calculated as:" << results.first << " miles" << std::endl;
      PlotPath(results.second[results.second.size()-1]); std::cout << menu;
    std::cout << "Time taken by function: " << duration.count()/1000 << " ms" << std::endl << std::endl;
    ...
    menu = "**************************************************************\n"
           "You could find your animation at src/lib/opt_3.avi.\n";
    std::cout << menu;
    std::cout << "Time taken by function: " << duration.count()/1000 << " ms" << std::endl << std::endl;
...
void MapUI::DynamicPrintMenu() {
  ...
  case '1':
  {
    ...
    y=ui.ScrollLongText("PRESS ANY KEY TO CONTINUE...",10,y);
    getchar();
    clear();
    DynamicPrintMenu();
    break;
  ...
  case '3':
  {
   menu =
    ...
    refresh();
    auto start = std::chrono::high_resolution_clock::now();
    auto results1 = map.CalculateShortestPath_Dijkstra(input1, input2);
    auto stop = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
    menu = "*************************Results******************************";
    y = ui.ScrollLongText(menu,10,y);
    if (results1.size() != 0) {
      std::string tmp="";
       ...
       y = ui.ScrollLongText(tmp+"\n", 10, y);
       menu = "The distance of the path with Djikstra algorithm is:" + std::to_string(map.CalculatePathLength(results1)) + " miles";
      y = ui.ScrollLongText(menu, 10, y);
      PlotPath(results1);
    } 
    else {
      y = ui.ScrollLongText("No path from the start point to the destination.", 10, y);
    }
    menu = "**************************************************************\n";
    y=ui.ScrollLongText(menu,10,y);
    y=ui.ScrollLongText("Time taken by Djikstra Algorithm: " + std::to_string(duration.count()/1000) + " ms",10,y);
    y=ui.ScrollLongText("PRESS ANY KEY TO CONTINUE...",10,y);
    getchar();
    clear();

    start = std::chrono::high_resolution_clock::now();
    auto results2 = map.CalculateShortestPath_Bellman_Ford(input1, input2);
    stop = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
    menu = "*************************Results******************************";
    y = ui.ScrollLongText(menu,10,0);
    if (results1.size() != 0) {
      std::string tmp="";
       for (auto x : results1) {
        ...
        y = ui.ScrollLongText(tmp +"\n", 10, y);
       menu = "The distance of the path with Bellman Ford Algorithm is:" + std::to_string(map.CalculatePathLength(results1)) + " miles";
      y = ui.ScrollLongText(menu, 10, y);
      PlotPath(results1);
    } 
    else {
      y = ui.ScrollLongText("No path from origin to destination.", 10, y);
    }
    menu = "**************************************************************\n";
    y=ui.ScrollLongText(menu,10,y);
    y=ui.ScrollLongText("Time taken by function: " + std::to_string(duration.count()/1000) + " ms",10,y);
  ...
  case '4':
  {
    ...
    if (results.second.size() != 0) {
      ...
      y = ui.ScrollLongText(menu,10,y);
      menu = "\nThe route distance is calculated as:" + std::to_string(results.first) + " miles\n";
      y = ui.ScrollLongText(menu,10,y);
      PlotPath(results.second[results.second.size()-1]);
    } else {
  ...
  case '5':
  {
    ...
    char input11[100];
    scanw("%s",input11);
    std::string input1 = input11;
    std::vector<double> square;
    square.push_back(atof(input1.c_str()));
    ...
    y = ui.ScrollLongText(menu,10,y);
    if (results == true){
      menu =  "there exists a cycle in the subgraph ";
      y = ui.ScrollLongText(menu,10,y);
    }
    else{
  ...
  case '6':
  {
    ...
    y = ui.ScrollLongText(menu,10,y);
    scanw("%s",input);
    std::string  locations_filename(input);
    menu = "Please input the dependencies filename:";
    y = ui.ScrollLongText(menu,10,y);
    scanw("%s",input);
    std::string dependencies_filename (input);
   // Read location names from CSV file
    std::vector<std::string> location_names;
    if (locations_filename == "") 
      location_names = {"Ralphs", "KFC", "Chick-fil-A"};
    ...
    if (result.size() > 0) {
      ...
      menu = "There is no topological sort for the given graph.\n";
      y = ui.ScrollLongText(menu,10,y);   
    }
    ...
    case '7':
  {
    ...
    menu = "*************************Results******************************";
    y = ui.ScrollLongText(menu,10,y);
    menu = "Find Nearby Results:";
    y = ui.ScrollLongText(menu,10,y);
    int cnt = 1;
    std::string temp;
    for (auto x : result) { 
      temp = map.data[x].name;       
      menu = cnt + " " + temp + "\n";
      y = ui.ScrollLongText(menu,10,y);
      cnt++;
    }
